...
describe("Test Crear Pais", () =>{
    ...
    describe("Casos de prueba: Clases de equivalencia validas", () => {
        ...
        test('CPU_01_CrearPais', async () => {
            ...
            await Request("Paises/Post", "post", data)
                .then(response => {expect(response.status).toBe(200); console.log(response.status)})
                .catch(error => { expect(error.status).toBe(200); console.log(error.status) })
        });
    ...
    describe("Casos de prueba: Clases de equivalencia invalidas", () => {
        ...
        test('CPU_02_CrearPais', async () => {
            ...
            await Request("Paises/Post", "post", data)
                .then(response => {expect(response.status).toBe(409 || 500); console.log(response.status)})
                .catch(error => {expect(error.status).toBe(409 || 500); console.log(error.status) })
        });
